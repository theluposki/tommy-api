...
var verify = (token) => {
  ...
  return jwt.verify(token, publicKey.toString(), options2);
};
...
function routes(app2) {
  ...
  app2.use(`${v1}/profiles`, validateToken, profileRouter_default);
}
...
// src/index.ts
console.log("\nPRODUCTION", config_default.production, "\n");
var serverHttp = http.createServer(app_default);
var options = config_default.certificates;
var serverHttps = https.createServer(options, app_default);
if (config_default.production === "true") {
  serverHttp.listen(config_default.app.PORT, () => {
    console.log(`[ APP ] Listening at \u{1F680}http://localhost:${config_default.app.PORT}`);
  });
} else {
  serverHttps.listen(config_default.app.PORT, () => {
